...
export async function GET(req: NextRequest) {
  ...
  const url = new URL(`${process.env.BACKEND_URL}/api/v1/ai/functions-list`);
  if (ideaId) url.searchParams.set("idea_id", ideaId);
  try {
    const { data } = await axios.get(url.toString(), { headers, timeout: 60_000 });
    return NextResponse.json(data);
  } catch (error: unknown) {
    const { message, status } = handleBackendError(error, "Failed to fetch functions list");
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { data } = await axios.post(
      `${process.env.BACKEND_URL}/api/v1/ai/functions-list`,
      body,
      { headers: { ...headers, "Content-Type": "application/json" }, timeout: 120_000 }
    );
    return NextResponse.json(data);
  } catch (error: unknown) {
    const { message, status } = handleBackendError(error, "Failed to generate functions list");
    return NextResponse.json({ error: message }, { status });
  }
}
