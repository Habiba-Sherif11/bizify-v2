...
export async function GET(req: NextRequest) {
  ...
  const url = new URL(`${process.env.BACKEND_URL}/api/v1/ai/go-to-market`);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { message, status } = handleBackendError(error, "Failed to generate go-to-market strategy");
    return NextResponse.json({ error: message }, { status });
  }
}
